...
Scenario: You're managing a pet store's inventory. The store keeps various types of pets, each identified by a unique ID. For each pet, the store records its type (like "dog", "cat", "fish"), and the quantity of that pet type currently in stock.
...
Task: Write a function updateInventory that takes three parameters: petStore (the inventory object), petID (the ID of the pet type), and additionalQuantity (the number of new pets of that type received by the store). The function should update the quantity of the specified pet type by adding the additionalQuantity. If the pet type with the given petID does not exist in the petStore, the function should add a new entry with petID, an assumed type of "Unknown", and the additionalQuantity as its stock.
